...
# Read a simple csv file and check the output
filename <- "sample.csv"
sample <- read.table(file.path(current_dir, filename, fsep="/"))

# ? Check what is inside the variable sample
sample

# ? Fixing the file reading
sample <- read.table(file.path(current_dir, filename, fsep="/"), sep=";", header=T)
