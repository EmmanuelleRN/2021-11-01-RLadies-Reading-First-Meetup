...
# ? Fix reading error by checking out the file header and separator used in this dataset. 
# You also probably need to change the file itself as well.
?read.table
titanic_df <- ...
...
# Try it with just filename as a parameter in read.csv and compare the output with 
# read.table
titanic_df <- ...
...
# Checking the classes of the columns. You don't need to understand apply function for now, 
# just run this line. 
# If you are interested, you can check what apply function does by running ?apply
apply(titanic_df, 2, class)
# As you can see, all of the column have character class, even though you expect e.g. 
# the Age column to be a number (numeric class in R).

# ? Specify column name for the 3rd column if you haven't change it in the file beforehand.
# Tip: colnames(...)[1] = "..."

# ? There are two very informative functions which you can use for data frames: 
# summary and str. Try it out!
...
# ? Read "titanic_test.csv" and merge it with the titanic_df data set you already have, 
# using function merge().
# As you will figure out soon, the columns in these two data files are different. 
# Tip: you can exclude one column from the data frame by: 
# titanic_df[, !names(titanic_df) %in% c(column_to_exclude)]
titanic_test_set <- ...
...
# ? Build a box plot with Age on y axis and Survived and Sex variables (combined) 
# on x axis for titanic_df dataset.
...
# Make a final conclusion how age and sex influenced the sirvivability of the passengers 
# in titanic wreck.